           ...
           if ScolsBitmas = pacman_col  and SfilsBitmas = pacman_fila then      
              PSJ<="0011";
               ...
               when "1000" => -- Solo btn3 está presionado   arriba 
                ...
                when "1001" =>     --arriba  derecha
                   if din(TO_INTEGER(not(ScolsBitmenos)))='0' then                                
                      pixel<= din(TO_INTEGER(not(ScolsBitmenos)));
                      azul   <= (others=>pixel);
                      verde <= (others=>pixel);
                      rojo  <= (others=>pixel);
                   end if;   
                 
                when "1010" =>     --arriba  izq       
                ...
                when "0110" =>     --ABAJO  izq 
                   if din(TO_INTEGER(not(SfilsBitmenos)))='0' then                                
                      pixel<= din(TO_INTEGER(not(SfilsBitmenos)));
                      azul   <= (others=>pixel);
                      verde <= (others=>pixel);
                      rojo  <= (others=>pixel);
                   end if;   
       
                when "0101" =>     --ABAJO  derecha
                   if din(TO_INTEGER(not(SfilsBitmenos)))='0' then                                
                      pixel<= din(TO_INTEGER(not(SfilsBitmenos)));
                      azul   <= (others=>pixel);
                      verde <= (others=>pixel);
